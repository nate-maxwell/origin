...
`package.yaml` from disk. Holds the package name, version, root path on disk,
...
The `package.yaml` file is a package's own declaration of what it contributes
to the environment. Each versioned package directory on disk contains a
`package.yaml` alongside the package code. Values in the `env` section are
...
derived from the version string in `package.yaml`. For a package named `mytool`
at version `2.3.0`:
