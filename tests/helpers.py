...
def make_mock_open(files: dict[str, dict]) -> MagicMock:
    """
    Return a mock for builtins.open that serves different YAML content per path.

    Args:
        files (dict[str, dict]): Maps file path strings to the dict that
            should be returned when that path is opened and YAML-decoded.
    Returns:
    ...
    def _open(path, *args, **kwargs):
        content = yaml.dump(normalized[str(Path(path))])
